...
subplot(SUB+2);
plot(outMapper.signals.values)
grid on;

subplot(SUB+3);
plot(outQPSKmod.signals.values,'-o')
grid on;
...
plot(simoutTime.signals.values, ReOut.signals.values.*g(simoutTime.signals.values-simoutTime.signals.values/lengthOfSymbol*Ts))
