% Programm audio_test2.m in dem eine Zerlegung und Rekonstruktion des
% Tiefpassanteils einer Zweikanal-Filterbank untersucht wird
...
hold on;  plot((0:nfft-1)/nfft, P);256
hold off;
...
r1 = filter(h0,1,x);      % Mit Analysefilter gefiltertes Signal
y1 = downsample(r1,2);    % Dezimiertes Signal
t1 = upsample(y1,2);      % Expandiertes Signal
v1 = filter(g0,1,t1);     % Mit Synthesefilter gefiltertes Signal
...
La = axis;   axis([La(1), max(nd*Ts), La(3:4)]);
%---------------------------------------------------
nd1 = min(nd)/2:max(nd)/2;
subplot(5,2,5), stem(nd1*2*Ts, y1(nd1));
ylabel('y1 (r1 dezim.)');   grid;
La = axis;   axis([La(1), max(nd1*2*Ts), La(3:4)]);

subplot(5,2,6), plot(nd1*2*Ts, y1(nd1));
ylabel('y1 (r1 dezim.)');   grid;
La = axis;   axis([La(1), max(nd1*2*Ts), La(3:4)]);
%---------------------------------------------------
