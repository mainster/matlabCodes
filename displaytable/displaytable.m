function [varargout] = displaytable(data, colheadings, wid, fms, rowheadings, fid, colsep, rowending)
...
%% Modified: 03-07-2015 M.DelBasso 
tabStr={};
rowCtr=1;
...
%            tabStr{rowCtr} = ...
%                  sprintf(fid, ['%s',colsep], repmat(' ', 1,rhwid));
            rowCtr = rowCtr + 1;
        end
        ...
        for i = 1:numel(colheadings)
            ...
            if i == numel(colheadings)
               ...
               tabStr{rowCtr} = ...
                     sprintf(fid, ['%s',colsep], str(1:wid(i)) );
               rowCtr = rowCtr + 1;
            end
    ...
    for i = 1:size(data,1)
        ...
        if ~isempty(rowheadings)
            ...
            tabStr{rowCtr} = ...
                  sprintf(fid, ['%',num2str(rhwid),'s',colsep], rowheadings{i});
            rowCtr = rowCtr + 1;
        ...
        for j = 1:size(data,2)
            ...
            if iscell(data)
                ...
                if j == size(data,2)
                   ...
                   tabStr{rowCtr} = ...
                        sprintf(fid, ['%s',colsep], str(1:wid(j)));
                   rowCtr = rowCtr + 1;
                end
            end
