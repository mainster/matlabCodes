...
void baseRateTask(void *arg)
{
  ...
  while (rtmGetErrorStatus(spiBitbang_complete_v2_2_M) == (NULL) ) {
    /* Wait for the next timer interrupt */
    MW_sigWait(&info.sigMask);
    spiBitbang_complete_v2_2_output();

    /* Get model outputs here */
    spiBitbang_complete_v2_2_update();
  }                                    /* while */
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(spiBitbang_complete_v2_2_M, 0);
  ...
  spiBitbang_complete_v2_2_initialize();
  ...
  if (rtmGetErrorStatus(spiBitbang_complete_v2_2_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(spiBitbang_complete_v2_2_M));
  }

  /* Disable rt_OneStep() here */

  /* Terminate model */
  spiBitbang_complete_v2_2_terminate();
  return 0;
}
