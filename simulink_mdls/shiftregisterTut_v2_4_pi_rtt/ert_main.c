...
void baseRateTask(void *arg)
{
  ...
  while (rtmGetErrorStatus(shiftregisterTut_v2_4_pi_M) == (NULL) ) {
    /* Wait for the next timer interrupt */
    MW_sigWait(&info.sigMask);
    shiftregisterTut_v2_4_pi_output();

    /* Get model outputs here */
    shiftregisterTut_v2_4_pi_update();
  }                                    /* while */
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(shiftregisterTut_v2_4_pi_M, 0);
  ...
  shiftregisterTut_v2_4_pi_initialize();
  ...
  if (rtmGetErrorStatus(shiftregisterTut_v2_4_pi_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(shiftregisterTut_v2_4_pi_M));
  }

  /* Disable rt_OneStep() here */

  /* Terminate model */
  shiftregisterTut_v2_4_pi_terminate();
  return 0;
}
