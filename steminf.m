function hh = steminf(varargin)
    %STEMINF   @@@MDG modified - Discrete sequence or "stem" plot.
    %   Replace dirac(t) |t=0 by a scalar 1
    %
    ...
    %   $Revision: 5.19.4.24 $  $Date: 2012/03/09 06:48:39 $
    ...
    if ishg2parent( varargin{:} )
        ...
        h = stemHGUsingMATLABClasses(cax, args{:});
...
function h = Lstemv6(cax,varargin)
    args = varargin;
    nin = length(args);
    ...
    while ischar(args{nin}),
        ...
        if ~isempty(v) && strncmpi(v, 'filled', vn)
            ...
            if ~isempty(msg),
                error(message('MATLAB:stem:UnknownOption', v));
            end
    ...
    % state by turning the
    % MATLAB:usev6plotapi:DeprecatedV6ArgumentForFilename and
    % MATLAB:usev6plotapihgusingmatlabclasses:IgnoringV6ArgumentForFilename
    % warnings off and on again.
    oldWarn(1) = warning('off', 'MATLAB:usev6plotapi:DeprecatedV6ArgumentForFilename');
    oldWarn(2) = warning('off', 'MATLAB:usev6plotapihgusingmatlabclasses:IgnoringV6ArgumentForFilename');
    try
