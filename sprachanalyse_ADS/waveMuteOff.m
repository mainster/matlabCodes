...
%         dir(PATHTO)
   ...
   DIVS = 20;
     ...
     for m=1:N
         ...
         lowvolLast(m) = find(diff( find(y(:,1) < LOWERTHAN_MEAN) )-1, 1, 'last'); 
        ...
        % Vorne wegschneiden.... % Hinten wegschneiden...
         y([1:lowvol(m)-PRE_DELAY, lowvolLast(m)+POST_DELAY:end]) = [];
...
%               end
%          end
     movefile(fullfile(PATHTO, ['' l{1,idx(m)}]),...
              fullfile(PATHTO, ['orig_' l{1,idx(m)}]));
     audiowrite(fullfile(PATHTO, ['' l{1,idx(m)}]), y, fs );   
     end
     
     if (nargin >= 3) && ischar(varargin{3}) && strcmpi(varargin{3},'plot')
         figure(99);
         ...
         for m=1:length(idx)
             subplot(SUB+m);
             plot(audioread( fullfile(PATHTO, ['orig_' l{1,idx(m)}]) )); hold all;
             plot(audioread( fullfile(PATHTO, ['' l{1,idx(m)}]) )); grid on;
             hold off;
             legend('orig','new short')
         end
     end      
%      dir(PATHTO)
