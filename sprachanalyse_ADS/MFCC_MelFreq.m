...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Nummer 1...9 
% sequenzen aus wave dateien einlesem
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for k=1:9
%    fss(k) = eval('audioinfo([PATHA, sprintf(''waveout_%i.wav'',1)]);');
    ...
    speechB{k} = eval( strrep([CMD '([PATHB, ''orig_wav_numberB_%i.wav''])'], '%i', num2str(k)));
    CMD = 'audioinfo';
    fss(k)  = eval( strrep([CMD '([PATHB, ''wav_numberB_%i.wav''])'], '%i', num2str(k)));
    fssB(k) = eval( strrep([CMD '([PATHB, ''orig_wav_numberB_%i.wav''])'], '%i', num2str(k)));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	for k=1:9
        ...
        lgpsd(k) = plot(ffyu/1e3, 10*log10(Pxx{k}),'b--','LineWidth',2); grid on;
        hold off;
        NOS{k} = num2str(k);
        if k==1
