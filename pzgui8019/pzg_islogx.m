function logscaled = pzg_islogx( dom_ndx, ploth_ndx )
% Returns the value of "Log" checkbox in specified figure.
...
if ( nargin < 2 ) || isempty(ploth_ndx) || ~isreal(ploth_ndx) ...
  || ~isequal( round(ploth_ndx), ploth_ndx )
  ploth_ndx = (1:5);
end  
ploth_ndx = ploth_ndx( ( ploth_ndx > 0 ) & ( ploth_ndx < 6 ) );
if isempty(ploth_ndx)
  ploth_ndx = (1:5);
end
  
log_cb_h = pzg_fndo( dom_ndx, ploth_ndx,'BodeAxesLinLogChkbox');
if ~isempty(log_cb_h)
  logscaled = get( log_cb_h(1),'value');
end
