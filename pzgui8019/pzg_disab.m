function  disab = pzg_disab
% Determines if either of the main GUIs' controls is disabled
% Format:  disab = pzg_disab
% where disab == 1 means that at least one of the main GUI's is disabled.
...
opt_h = pzg_fndo([1;2],(1:14),'file_menu');
if ~isempty(opt_h)
  opt_h_enable = get( opt_h(ishandle(opt_h)),'enable');
  if any( strcmp( opt_h_enable,'off') )
    disab = 1;
  end
end
