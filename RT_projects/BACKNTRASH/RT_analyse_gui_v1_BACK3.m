...
%    figure_handle = figure('Toolbar','none');
    ...
    for i=1:6
        ...
        text(0.5,0.5, na(i))
        set(handles.ha(i),'Position',str2num(axpos2{i})*ppm);
    end
%     i=4;
...
%     text(0.5,0.5, na(i))
    ...
    slidertext={'Re','Im'};
    sliderCallbacks={   {@surfxlim, handles.ha( pos.wok )},...
                        {@surfylim, handles.ha( pos.wok )} };
    for i=1:2
...
%                         'Callback', {@surfxlim, handles.ha( pos.wok )});                 
    ...
    posui=[get(handles.ha(pos.bodeMag),'Position') get(handles.ha(pos.nyquist),'Position')]/ppm;
    set(hui,'position',[posui(1) posui(6) DimAxis(3:4)]*ppm)

    align([handles.ha(pos.pzmap) handles.ha(pos.bodeMag)],'Vertical','top')
...
%         handles.md.ranges.bode=[min(wout) max(wout)*10];
%         handles.md.ranges
        ...
        nyq(2,:)=squeeze(im);
        nyFact=1.4;
        handles.md.ranges.nyquist=[ min(re)*nyFact max(re)*nyFact,...
...
%            setRng=@(h,x) set(h, 'XLim',x(1,:));
            ...
            set(handles.ha([pos.bodeMag, pos.bodePhase]),'XLimMode','manual','YLimMode','auto');
...
toc
        ...
        % Bode
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        win=logspace(-2,3,100);
        [mag,phase,wout]=bode(DI,win);
        mag=squeeze(mag);
        phase=squeeze(phase);
        handles.md.ranges.bode=[];
        axes(handles.ha( pos.bodeMag ));
        
        hbode(1)=semilogx(handles.ha( pos.bodeMag ),wout,20*log10(mag));
        xlim(handles.ha(pos.bodeMag), [1e-2 1e3]);
        set(handles.ha(pos.bodeMag),'XTickLabel',[]);
        grid on;
        
        axes(handles.ha( pos.bodePhase ));
        set(gca,'XLimMode','manual');
        hbode(2)=semilogx(handles.ha( pos.bodePhase ),wout,phase);
        xlim(handles.ha(pos.bodePhase), [1e-2 1e3]);
%        set(handles.ha(pos.bodeMag),'XTickLabel',[]);
        grid on;
toc
%         hold on
