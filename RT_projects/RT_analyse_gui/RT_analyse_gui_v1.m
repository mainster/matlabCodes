...
%     i=4;
% %    h.ha(i)=BodePaper(1e-3,1e3,-200,10,-180,90);
%     h.ha(i)semilogx(wout,20*log10(mag))
%     i=5;
%     h.ha(i)= axes('Units','Pixels','Position',DimAxis*ppm, 'Tag',na{i});
%     text(0.5,0.5, na(i))
    set(h.ha(i),'Position',str2num(axpos2{i})*ppm);
...
%                         'Callback', {@surfxlim, h.ha( pos.wok )});                 
    ...
    set(sli(3:4),'Min',-5,'Max',5,'Value',0);
    
    set(sli,'pos',[5 30 5.5 48]*ppm);
    align([htfnum sli],'Horizontal','top')
    align([sli],'fixed',0.8*ppm,'Vertical')
    ...
    align([h.ha(pos.pzmap) h.ha(pos.bodeMag)],'Vertical','top')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function ret = tfeditbox_parser(hObject, eventdata, h)
        ...
        % -----------------------------------------------------
%        if isempty(strfind(eventdata.swParam,'_'))
...
%            setRng=@(h,x) set(h, 'XLim',x(1,:));
            ...
            set(h.ha([1,2,5,6]),'XLimMode','manual','YLimMode','manual');
...
%        set(h.ha(pos.bodeMag),'XTickLabel',[]);
        grid on;
toc
%         hold on
%         bpl=bodeplot(DI)
...
%        get(source)
    ...
    function sliderX_Callback(hObj,event,ax) 
        h=guidata(f);
