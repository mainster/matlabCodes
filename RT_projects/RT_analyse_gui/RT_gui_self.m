...
DimPoleEdit=[45+deltaVert-6*(2.2+0.25*vspace/ppc), 8, 2.2, 0.9]*ppc;
...
editsv(6,2)=editsv(6,2)-4*ppc;

editsv(7,:)=DimPoleEdit;
editsv(7,3)=editsv(7,3)*1.2;
editsv(7,2)=editsv(7,2)-4*ppc;
editsv(7,1)=editsv(7,1)+3*ppc;
...
posax(3,2)=posax(2,2)+hiaxis+2*hspace;
...
f = figure('Visible','on','Position',DimFigure);
    set(f,'Units','Centimeters','WindowStyle','Docked');
%  Construct the components.
    ...
    boxposden=[45+deltaVert-6*(1.6+vspace/ppc), 12, 9.1, 1.1]*ppc
    boxposnum=[45+deltaVert-6*(1.6+vspace/ppc), 13, 9.1, 1.1]*ppc
    ...
    ha(3) = axes('Units','Pixels','Position',posax(1,:)); 
    ha(4) = axes('Units','Pixels','Position',posax(2,:)); 
    ha(1) = axes('Units','Pixels','Position',posax(3,:)); 
    ha(2) = axes('Units','Pixels','Position',posax(4,:)); 

    align([hbut,hpoedit,hzoedit,htfnum],'East','None');
