...
SIMFILES={  'Galvo_sys_Cdisc_Pcont_cc_nosub_GUT_v63',...
            'GalvoModel_v44_simply',...
...
s=struct;
FB_EN=0;
s0=linmodHighLevel(gcs,s,'dlin');
FB_EN=1;
st=linmodHighLevel(gcs,s,'dlin');
...
f3 = figure(3);
step(feedback(vname(evalc('SYS1'),1)));
hold all;
step(feedback(vname(evalc('SYS2'),1)));
hold off;
grid on;
legend('Cp1','Cpi1');

f4 = figure(4);
h1=bodeplot(feedback(SYS1,1));
hold all;
h2=bodeplot(feedback(SYS2,1));
legend('Cp1','Cpi1');
hold off;
...
%%
% *********************************************************************
%               Multiplot von Gl* approx. systemen
%           	FIT ?% abhängig
% *********************************************************************

%load_system('Loop_detailed_model_comp_cc_vccc_v13');
clear G2 n;
...
% 
% [zs ps k]=zpkdata(G1);
% disp('simulink schimpft wegen hoher systemsteifigkeit...')
% G1
% sprintf(['%s\nzeros: %s \npoles: ' repmat('%i  ',1,length(ps{:})) '\ngain: %.3e\n%s\n'],...
%     ds, 'keine', ps{1}(:)', k, ds)

%normto=cellfun(@min, ps, 'UniformOutput', false)

s_perc=Gl{end-2}([1:end],4,1);
...
f1=figure(1);
hold all
bodeplot(Gl{34}{ind,1,1},optb)
legend(legStr(1:end-1),'interpreter','none')
hold off

f2=figure(2);
hold all
nyquist(Gl{34}{ind,1,1},optn)
legend(legStr(1:end-1),'interpreter','none')
hold off

legStr=strsplit(sprintf('Gl{33}{%i,1,1}#\n',ind),'#')';
...
P(k,:) = ss(Plant.a, Plant.b, Plant.c, Plant.d, 'u', u_, 'y', y_);         
[num, den] = ss2tf(Plant.a, Plant.b, Plant.c, Plant.d);
if FB_EN == 0
    P(k,:).Name = [Plant.filename '_G0']
else
    P(k,:).Name = [Plant.filename '_Gtot']
end

NUM=mat2cell(num, [ones(1, size(num,1))], size(num,2));
Ptf(k) = tf(NUM, den, 'u', u_, 'y', y_); 
Ptf(k).Name = Plant.filename;
...
disp(sprintf(['Poles:\t\t' repmat('%.1f  ',1,length(ps))],ps(1:end)))
